...
class HrAdvance(models.Model):
    ...
    name = fields.Char(string="Advance Name", default="/", readonly=True, help="Name of the Advance")
    date = fields.Date(string="Requisition Date", default=fields.Date.today(), readonly=True, help="Date")
    ...
    date_payment = fields.Date(string="Payment Date",  default=fields.Date.today(), help="Date of "
                                    "the "
                                    "paymemt")
    payment_date = fields.Date(string="Deduction Start Date", required=True, default=fields.Date.today(), help="Date of "
                                                                                                             "the "
                                                                                                             "paymemt")
    advance_lines = fields.One2many('hr.advance.line', 'advance_id', string="Advance Line", index=True)
    ...
    job_position = fields.Many2one('hr.job', related="employee_id.job_id", readonly=True, string="Job Position",
                                   help="Job position")
    loan_amount = fields.Float(string="Advance Amount", required=True, help="Advance amount")
    ...
    def create(self, values):
        loan_count = self.env['hr.advance'].search_count(
    ...
    def unlink(self):
        for loan in self:
            if loan.state not in ('draft', 'cancel'):
                raise UserError(
                    'You cannot delete an advance which is not in draft or cancelled state')
...
class InstallmentLine(models.Model):
    ...
    advance_id = fields.Many2one('hr.advance', string="Advance Ref.", help="Advance")
...
class HrEmployeeInh(models.Model):
    ...
    employee_type = fields.Selection([('internee', 'Internee'),
                                      ('probation', 'Probation'),
                                      ('permanent', 'Permanent'),
                                      ('contract', 'Contract'),
